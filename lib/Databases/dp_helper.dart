...
class DBHelper extends GetxService{
 /// In Dart, the underscore (_) at the beginning of a variable or method name indicates private access.
  ...
  _onCreate(Database db, int version){
    ...
    db.execute("CREATE TABLE $orderMasterTableName(order_master_id TEXT PRIMARY KEY, order_master_date TEXT, order_master_time TEXT,shop_name TEXT,owner_name TEXT, phone_no TEXT,brand TEXT,total TEXT, credit_limit TEXT, requiredDelivery TEXT)");
    db.execute("CREATE TABLE $orderDetailsTableName (order_details_id TEXT PRIMARY KEY, order_detail_date TEXT, order_details_time TEXT, order_master_id TEXT, product TEXT, quantity TEXT, in_stock TEXT, rate TEXT, amount TEXT, FOREIGN KEY(order_master_id) REFERENCES $orderMasterTableName(order_master_id))");
    ...
    db.execute("CREATE TABLE $returnFormDetailsTableName(return_details_id TEXT PRIMARY KEY, return_details_date TEXT, return_details_time TEXT, return_master_id TEXT, item TEXT, qty TEXT, reason TEXT, FOREIGN KEY(return_master_id) REFERENCES $returnFormMasterTableName(return_master_id))");
    db.execute("CREATE TABLE $recoveryFormTableName(recovery_id TEXT PRIMARY KEY, recovery_date TEXT, recovery_time TEXT shop_name TEXT,current_balance TEXT,cash_recovery TEXT,new_balance TEXT,date TEXT)");
