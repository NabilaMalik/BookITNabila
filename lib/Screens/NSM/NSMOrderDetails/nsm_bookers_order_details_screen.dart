...
class NsmBookersOrderDetailsScreen extends StatefulWidget {
  const NsmBookersOrderDetailsScreen({super.key});

  @override
  _NSM_SM_StatusState createState() => _NSM_SM_StatusState();
}

class _NSM_SM_StatusState extends State<NsmBookersOrderDetailsScreen> {
  ...
  Future<bool> _fetchAndSaveData() async {    await  Config.fetchLatestConfig();
    final url =
        '${Config.getApiUrlServerIP}${Config.getApiUrlERPCompanyName}${Config.getApiUrlNsmUserOrder}$user_id';
      ...
      child: GestureDetector(
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => NsmBookersBookingDetailsScreen(booker: booker),
            ),
          );
        },
        child: Card(
