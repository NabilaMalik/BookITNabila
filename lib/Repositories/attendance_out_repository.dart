...
class AttendanceOutRepository{
  ...
  Future<List<AttendanceOutModel>> getAttendanceOut() async{
    ...
    List<Map> maps = await dbClient.query(attendanceOutTableName,columns: ['id','date','timeOut','userId','totalTime','latOut,''lngOut','totalDistance','address' ]);
  ...
  Future<int> add(AttendanceOutModel attendanceoutModel) async{
    var dbClient = await dbHelper.db;
    return await dbClient.insert(attendanceOutTableName, attendanceoutModel.toMap());
  }

  Future<int> update(AttendanceOutModel attendanceoutModel) async{
    var dbClient = await dbHelper.db;
    return await dbClient.update(attendanceOutTableName, attendanceoutModel.toMap(),
